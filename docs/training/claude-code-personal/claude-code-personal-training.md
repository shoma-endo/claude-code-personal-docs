...
講義当日までに以下の **3 つ** の準備を済ませてください。**推奨順は ② → ③ → ①** です（エディターを入れてから、用語を理解し、`claude` を確認する流れ）。**講義用 sample-data の取得は Session 2 の §5 のあと**（[講義用サンプルデータの準備](#講義用サンプルデータの準備)）で行います。事前準備では ZIP は不要です。
...
- **§1〜§5** 概念と安全（**sample-data はまだ不要**）
- **[講義用サンプルデータの準備](#講義用サンプルデータの準備)**（§5 の直後） ZIP を取得し `sample-data` を開く。**§6 以降はここで止めず、この準備が終わってから進む**
- **§6〜§11** 同じフォルダで `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **§12〜§14** 同梱ファイルの差し替え・要約・`CLAUDE.md` 追記の演習
...
- 講義用のダミーデータ（Session 2 の **§5 のあと**に配布する **sample-data.zip** の中身。§2 時点ではまだ取得していなくてよい）
- 公開済み情報
- 個人名や会社名を伏せた業務例
...
エディターで `~/claude-code-personal` を開いても構いません。**§6〜§14 では、上記フォルダのまま**進めます。
...
```text
sample-data/                   # 本講義の作業フォルダ（§5 のあとで準備。コピー後は任意の名前でも可）
├── CLAUDE.md                  # 共有ルール
