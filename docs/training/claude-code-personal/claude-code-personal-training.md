...
| Session 2 | Claude Code の基礎・基本操作と設定 | 起動・ファイル操作・コマンド・Skills / Hooks・コンテキスト管理を理解する |
...
講義当日までに以下の **4 つ** の準備を済ませてください。**推奨順は ② → ③ → ① → ④** です（エディターを入れてから、用語を理解し、`claude` を確認し、最後にサンプルデータをコピーする流れ）。
...
- [ ] （推奨）講義用サンプルデータ（§④）を取得し、`sample-data`（またはコピー先）をエディターで開ける
...
エディターで `~/claude-code-personal` を開いても構いません。**Session 2 は §4 以降、同じ作業フォルダで進めます。**

含まれる主なファイル:

- `CLAUDE.md` — Session 2 **§10** 以降で参照するプロジェクトルール
- `.claude/skills/meeting-summary/SKILL.md` — **§14**・**§18** の Skill 例
- `.claude/settings.json` — **§12** の設定ファイル例
- `meeting-memo.txt` — **§18** の議事メモ（ダミー）
- `examples/about-me.md` — **§17** の完成例（演習**後**に参照）
- `examples/meeting-summary.md` — **§18** の要約例
...
> [!NOTE]
> 本节の本文は現在作成中です。Session 2 以降のハンズオンに入る前の概念整理パートです。

---

## Session 2 — Claude Code の基礎・基本操作と設定

> **ゴール**: Claude Code の概要、安全な使い方、起動・ファイル操作、各種コマンド、Skills / Hooks、コンテキスト管理を理解する

> [!IMPORTANT]
> **本講義は Claude Code の CLI（ターミナル版）を前提** に進めます。**エディター**でファイルを確認しながら、**ターミナル**（エディター内蔵で可）で `claude` を起動して指示を出します。Desktop アプリ版だけに慣れている場合も、講義中はエディター + ターミナルの組み合わせで体験してください。
...
> [!IMPORTANT]
> **講義用サンプルデータは Session 2 の §4 以降ずっと同じフォルダで使います。** エディターで `CLAUDE.md` や Skill ファイルを**実際に開きながら**学び、§17 以降ではファイル操作の演習に入ります。
...
#### ターミナルとエディターの役割（本講義の進め方）

初心者向けの詳細は **事前準備 §③** にまとめています。

- **§4 以降ずっと同じフォルダ**: 事前準備 §④ の **`sample-data`（またはコピー先）** をエディターで開いたまま進める
- **§10〜§15**: ルートの `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **§17〜§19**: 同じフォルダでファイル作成・要約・ルール追記の演習

1. **エディター**で `sample-data`（または `~/claude-code-personal`）を開く（§④ で済ませておく）
2. **ターミナル**で同じパスに移動し `claude` を起動する
3. ターミナルで日本語の依頼を送る
4. Claude Code がファイルを作成・更新する（`CLAUDE.md` は起動時に自動読み込み）
5. **エディター**で結果を確認する

| 操作の種類 | 使う場所 | 例 |
|---|---|---|
| Claude への指示 | ターミナル | 「`about-me.md` を作成して」 |
| 作成ファイルの確認 | エディター | 左の一覧から `about-me.md` をクリック |
| フォルダの移動 | ターミナル | `cd claude-code-personal` |
| 手動でのちょっとした修正 | エディター | 誤字を直す、メモを追記する |

> [!TIP]
> **ターミナル = 指示を出す場所**、**エディター = 成果物を見る場所** と覚えると混乱しにくいです。Claude Code だけがターミナルで動き、あなたはエディターで結果をチェックする、という分担です。
...
**エディター内のターミナル** から起動するのが本講義の標準です（事前準備 §③ 参照）。**事前準備 §④** の **`sample-data` フォルダ**（またはコピー先）を開いた状態で進めます。

1. エディターで **`sample-data`** を開く（コピーした場合は `~/claude-code-personal` など）
2. ターミナルパネルを表示する（`` Ctrl + ` ``）
3. ターミナルのカレントが **ルートに `CLAUDE.md` があるフォルダ** であることを確認してから `claude` を起動する

```bash
cd ~/claude-code-personal   # コピーした場合の例。解凍先の sample-data でも可
claude
```

> [!NOTE]
> macOS / Windows 標準の「ターミナル」「PowerShell」アプリから起動しても動作します。ただし、**同じフォルダをエディターでも開いておく** と、Claude が作ったファイルをすぐ確認できて学習しやすくなります。
...
> [!NOTE]
> **ルールの追記実習**は **§19** で行います（ゼロから作るのではなく、既存の `CLAUDE.md` に 1 行足す）。

### 11. プロジェクトルートと `.claude/` ディレクトリ
...
### 12. お願いとガード（CLAUDE.md / settings / hooks）
...
§4 以降と同じフォルダで `claude` を起動したまま進めます。エディターのサイドバーで次が見えることを確認してください。

| パス | 役割 |
|---|---|
| `CLAUDE.md` | §10 で確認したプロジェクトルール |
| `.claude/skills/meeting-summary/SKILL.md` | §14 で確認した Skill |
| `meeting-memo.txt` | §18 の要約演習用 |
| `examples/` | 演習**後**の模範解答（演習中は開かない） |

ターミナルが別のフォルダを指しているときは、再度 `cd` してから続けます。
...
> [!TIP]
> 事前準備 §④ のサンプルには、すでに **`meeting-memo.txt`**（拡張版のダミー議事メモ）が入っています。ある場合は「作成」ステップを飛ばし、いきなし要約の依頼から始めて構いません。模範解答は `examples/meeting-summary.md` です（演習後に参照）。

**サンプルがない場合** — 次の内容で `meeting-memo.txt` を新規作成させます。
...
### 19. CLAUDE.md にルールを追記する

§10 で確認した **既存の `CLAUDE.md`** に、自分用のルールを 1 行足します。
