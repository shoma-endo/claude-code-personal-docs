...
講義当日までに以下の **3 つ** の準備を済ませてください。**推奨順は ② → ③ → ①** です（エディターを入れてから、用語を理解し、`claude` を確認する流れ）。**講義用 sample-data の取得は Session 2 の §6 のあと**（[講義用サンプルデータの準備](#講義用サンプルデータの準備)）で行います。事前準備では ZIP は不要です。
...
- **§1〜§6** 概念と安全（**sample-data はまだ不要**）
- **[講義用サンプルデータの準備](#講義用サンプルデータの準備)**（§6 の直後） ZIP を取得し `sample-data` を開く。**§7 以降はここで止めず、この準備が終わってから進む**
- **§7〜§12** 同じフォルダで `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **§13〜§15** 同梱ファイルの差し替え・要約・`CLAUDE.md` 追記の演習
...
- 講義用のダミーデータ（Session 2 の **§6 のあと**に配布する **sample-data.zip** の中身。§3 時点ではまだ取得していなくてよい）
- 公開済み情報
- 個人名や会社名を伏せた業務例
...
> [!IMPORTANT]
> **§7 以降は、この準備が終わるまで進めないでください。** ここから **sample-data** フォルダ内のファイルを開きます。演習用の **ダミーファイル一式** を配布しています（実在の顧客名・個人名は含みません）。
>
...
エディターで `~/claude-code-personal` を開いても構いません。**§7〜§15 では、上記フォルダのまま**進めます。

含まれる主なファイル

- `CLAUDE.md` — **§7** 以降で参照するプロジェクトルール
- `about-me.md` — **§13** の自己紹介（ダミー。**差し替え**演習用）
- `meeting-memo.txt` — **§14** の議事メモ（ダミー）
- `.claude/skills/meeting-summary/SKILL.md` — **§11**・**§14** の Skill 例
- `.claude/settings.json` — **§9** の設定ファイル例
...
### 8. プロジェクトルートと `.claude/` ディレクトリ

Claude Code の設定は、**プロジェクトルート**と **`.claude/` 配下**に置きます。講義用サンプルでは次の構成です。

```text
sample-data/                   # 本講義の作業フォルダ（§6 のあとで準備。コピー後は任意の名前でも可）
├── CLAUDE.md                  # 共有ルール
...
### 9. お願いとガード（CLAUDE.md / settings / hooks）
...
§7 で確認した **既存の `CLAUDE.md`** に、自分用のルールを 1 行足します。
