...
講義当日までに以下の **4 つ** の準備を済ませてください。**推奨順は ② → ③ → ① → ④** です（エディターを入れてから、用語を理解し、`claude` を確認し、最後にサンプルデータを開く流れ）。**Session 2 以降は、④ まで完了していることが前提**です。
...
| サイドバー（左） | フォルダ内のファイル一覧。Claude が作った `about-me.md` などがここに現れる |
...
- [ ] 講義用サンプルデータ（§④）を取得し、`sample-data`（またはコピー先）をエディターで開い、ルートに `CLAUDE.md` と `meeting-memo.txt` が見える
...
演習用の **ダミーファイル一式** を配布しています。実在の顧客名・個人名は含みません。

| パス | 内容 |
|---|---|
| [sample-data/README.md](./sample-data/README.md) | ダウンロード手順・使い方 |
| [sample-data/](./sample-data/) | **ワークスペース本体**（`CLAUDE.md` がこのフォルダの直下にある） |
| [GitHub 上の sample-data](https://github.com/shoma-endo/claude-code-personal-docs/tree/main/docs/training/claude-code-personal/sample-data) | ZIP 取得・フォルダの確認 |

> [!IMPORTANT]
> **エディターで開くのは `sample-data` フォルダです。** ルートに `CLAUDE.md` と `meeting-memo.txt` が見えていれば正しい階層です。

**手順**:

1. 上記 GitHub から **sample-data** フォルダを ZIP でダウンロードするか、配布されたアーカイブを解凍する
...
エディターで `~/claude-code-personal` を開いても構いません。**Session 2 では事前準備 §④ の作業フォルダのまま**進めます。

含まれる主なファイル:

- `CLAUDE.md` — Session 2 **§8** 以降で参照するプロジェクトルール
- `.claude/skills/meeting-summary/SKILL.md` — **§12**・**§15** の Skill 例
- `.claude/settings.json` — **§10** の設定ファイル例
- `meeting-memo.txt` — **§15** の議事メモ（ダミー）
- `examples/about-me.md` — **§14** の完成例（演習**後**に参照）
- `examples/meeting-summary.md` — **§15** の要約例
...
| 「動く AI」の考え方 | 実際に `claude` でファイルを作り、エージェントループを体験する |
...
> [!IMPORTANT]
> **講義用サンプルデータは Session 2 全体で同じフォルダを使います。** エディターで `CLAUDE.md` や Skill ファイルを**実際に開きながら**学び、§14 以降ではファイル操作の演習に入ります。
...
エディターとターミナルの役割・開き方は **事前準備 §③**、作業フォルダは **§④** で済ませています。Session 2 では次の流れで進めます。

- **§8〜§13**: ルートの `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **§14〜§16**: 同じフォルダでファイル作成・要約・ルール追記の演習
...
- 講義用のダミーデータ（[sample-data](./sample-data/) からダウンロード可）
- 公開済み情報
- 個人名や会社名を伏せた業務例
...
```text
sample-data/                   # 本講義の作業フォルダ（事前準備 §④。コピー後は任意の名前でも可）
├── CLAUDE.md                  # 共有ルール
...
```text
about-me.md を作成してください。
担当業務・得意分野・今取り組んでいることを含め、講義用のダミーデータで構いません。
```
...
- `about-me.md` が作成される
- **エディター**の左サイドバーにファイルが現れる
- クリックして編集エリアで内容を確認できる

### 15. メモファイルを要約する

事前準備 §④ の **`meeting-memo.txt`** を要約します。模範解答は `examples/meeting-summary.md`（演習後に参照）。
...
| 別フォルダで作業している | ターミナルのカレントがずれている | 事前準備 §④ のフォルダに `cd` してから再開 |
...
続けて、CLAUDE.md に業務ルールを追記します（Session 2 で作成したファイルに追記して構いません）。
