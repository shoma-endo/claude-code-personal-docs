...
1. Claude Code の基礎・基本を理解し、ファイル操作・各種コマンド・Skills / Hooks・コンテキスト管理まで操作できる
...
| ブロック | タイトル | 到達目標 |
|---|---|---|
| Session 1 | Claude Code の基礎・基本操作と設定 | 起動・ファイル操作・コマンド・Skills / Hooks・コンテキスト管理を理解する |
...
- [ ] （Session 2 用・任意）Node.js（`npm` / `npx` が使える環境）を用意した（Lark CLI は Session 2 の §9 でインストール。cc-secretary より先に入れない）
...
| パス | 内容 |
|---|---|
| [sample-data/README.md](./sample-data/README.md) | ダウンロード手順・使い方（リポジトリ内） |
| [sample-data/](./sample-data/) | **ワークスペース本体**（`CLAUDE.md` がこのフォルダの直下にある） |
| [GitHub 上の sample-data](https://github.com/shoma-endo/claude-code-personal-docs/tree/main/docs/training/claude-code-personal/sample-data) | ブラウザからフォルダを開く・ZIP 取得 |

> [!IMPORTANT]
> **エディターで開くのは `sample-data` フォルダです。** その中にさらに `claude-code-personal` という子フォルダがある構成ではありません。ルートに `CLAUDE.md` と `meeting-memo.txt` が見えていれば正しい階層です。

**パターン A — リポジトリ内の `sample-data` をそのまま開く**（clone 済み）:

1. エディターで `docs/training/claude-code-personal/sample-data` を開く
2. ターミナルで同じフォルダに移動し `claude` を起動

**パターン B — ホームにコピーして使う**（パスを短くしたい場合）:

```bash
cp -r docs/training/claude-code-personal/sample-data ~/claude-code-personal
```

エディターで `~/claude-code-personal` を開き、内蔵ターミナルで `claude` を起動できることを確認してください。**Session 1 は §4 以降、このフォルダだけで進めます**（前半・後半でフォルダを切り替えません）。

含まれる主なファイル:

- `CLAUDE.md` — Session 1 **§10** 以降で参照するプロジェクトルール
...
| パート | 節 | 内容 |
|---|---|---|
| **共通** | 事前準備 §④ | **`sample-data` を開く**（または `~/claude-code-personal` へコピー）し、Session 1 全体の作業フォルダとする |
| **前半** | §1〜§9 | 概要、CLI 前提、安全、**サンプルフォルダで**起動・対話・許可、エージェントループ |
| **前半** | §10〜§15 | 同フォルダ内の **`CLAUDE.md`・`.claude/skills` 等の実物** で設定の説明・確認 |
| **後半** | §16〜§19 | 作成・要約・`CLAUDE.md` 追記 |
| **締め** | §20 | 振り返り（コマンド一覧は資料参照） |

> [!IMPORTANT]
> **講義用サンプルデータは Session 1 の最初（§4）から使います。** 前半は「説明だけ」ではなく、エディターで `CLAUDE.md` や Skill ファイルを**実際に開きながら**理解します。後半は同じフォルダでファイル操作の演習に入ります。

> [!NOTE]
> **本講義の Session 1 は 2 時間（質疑応答含む）** です。
...
| 優先 | 調整する内容 | 節 |
|:---:|---|---|
| 1 | コマンドリファレンスは資料のみ | §20 |
| 2 | `/clear` 後の再確認を飛ばす | §19 |
| 3 | 要約依頼の Skill 参照行を飛ばす | §18 |
| 4 | Skill 説明依頼を口頭説明に置き換え | §14 |
| 5 | CLI vs Desktop 詳細表を要点のみ | §1 |
| 6 | `about-me.md` 作成を飛ばし、要約・追記のみ | §17 |
...
- **前半（§10〜§15）**: ルートの `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **後半（§17〜§19）**: 同じフォルダでファイル作成・要約・ルール追記の演習
...
- Session 2 の **AI 秘書（cc-secretary）** の `/plugin` や **Lark CLI** は、ターミナルから Claude Code がコマンドを実行する前提で設計されている
...
# リポジトリ内の sample-data を開いている場合の例
cd docs/training/claude-code-personal/sample-data
claude
```
...
> [!NOTE]
> **ルールの追記実習**は後半 **§19** で行います（ゼロから作るのではなく、既存の `CLAUDE.md` に 1 行足す）。
...
## Session 1 後半 — ファイル操作の実践

> **ここから後半です。** 作業フォルダは **前半と同じ** `sample-data`（またはコピー先）です。`CLAUDE.md` と `.claude/` はすでに入っているので、フォルダの切り替えや作り直しは不要です。
...
| パス | 役割 |
|---|---|
| `CLAUDE.md` | 前半 §10 で確認したプロジェクトルール |
| `.claude/skills/meeting-summary/SKILL.md` | 前半 §14 で確認した Skill |
| `meeting-memo.txt` | §18 の要約演習用（これから使う） |
...
前半 §10 で確認した **既存の `CLAUDE.md`** に、自分用のルールを 1 行足します。
...
講義の締めでは必要なコマンドだけ触れます。一覧は講義後の参照用です。
