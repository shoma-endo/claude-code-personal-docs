...
- `.claude/skills/meeting-summary/SKILL.md` — **§12**・**§16** の Skill 例
- `.claude/settings.json` — **§10** の設定ファイル例
- `meeting-memo.txt` — **§16** の議事メモ（ダミー）
- `examples/about-me.md` — **§15** の完成例（演習**後**に参照）
- `examples/meeting-summary.md` — **§16** の要約例
...
> [!IMPORTANT]
> **講義用サンプルデータは Session 2 全体で同じフォルダを使います。** エディターで `CLAUDE.md` や Skill ファイルを**実際に開きながら**学び、§15 以降ではファイル操作の演習に入ります。
...
- **事前準備 §④ の作業フォルダ**をエディターで開いたまま進める
- **§8〜§13**: ルートの `CLAUDE.md` や `.claude/skills/` をエディターで見ながら設定の仕組みを学ぶ
- **§15〜§17**: 同じフォルダでファイル作成・要約・ルール追記の演習

事前準備で `claude` が起動し、会話できる状態になっている前提です。

1. **エディター**で `sample-data`（または `~/claude-code-personal`）を開く（事前準備 §④）
2. **ターミナル**で同じフォルダから依頼を送る
3. Claude Code がファイルを作成・更新する（`CLAUDE.md` は起動時に自動読み込み）
4. **エディター**で結果を確認する
...
> [!NOTE]
> **ルールの追記実習**は **§17** で行います（ゼロから作るのではなく、既存の `CLAUDE.md` に 1 行足す）。
...
事前準備 §④ の作業フォルダで `claude` を起動したまま進めます。エディターのサイドバーで次が見えることを確認してください。

| パス | 役割 |
|---|---|
| `CLAUDE.md` | §8 で確認したプロジェクトルール |
| `.claude/skills/meeting-summary/SKILL.md` | §12 で確認した Skill |
| `meeting-memo.txt` | §16 の要約演習用 |
