...
| 全コーディング規約を CLAUDE.md に貼り付ける | 長すぎてコンテキストを圧迫する | `.claude/rules/coding.md` に分割し、`paths` で対象フォルダに限定する |
...
初回導入では、まず CLAUDE.md から始め、定型手順が必要になったら Skills を追加します。Hooks は運用が増えてから段階的に導入します。
