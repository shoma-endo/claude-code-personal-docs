...
講義当日までに以下の 3 つの準備を済ませてください。**推奨順は ② → ③ → ①** です（エディターを入れてから、用語を理解し、最後に `claude` の動作確認をする流れ）。
...
| 3 | 個人利用の安全ルール | 講義 |
| 4 | 作業フォルダの用意、起動、初回対話、許可・停止 | 実習 |
| 5 | チェックポイント、エージェントループ | 講義 |
| 6 | ファイル作成・読取・編集（講義用フォルダ内） | 実習 |
...
初心者向けの詳細は **事前準備 §③** にまとめています。Session 1 では次の流れを繰り返します。

1. **エディター**で講義用フォルダを開く
2. **ターミナル**（エディター下部）で `claude` を起動する
3. ターミナルで日本語の依頼を送る
4. Claude Code がファイルを作成・更新する
5. **エディター**のファイル一覧・編集画面で結果を確認する
...
> [!NOTE]
> **読み進め方**: 初めての方は、次の **§10（作業フォルダ）を先に実施** してから本節に戻るとスムーズです。すでに事前準備 §③ でフォルダを開き `claude` が動いていれば、そのフォルダのまま続行して構いません。

**エディター内のターミナル** から起動するのが本講義の標準です（事前準備 §③ 参照）。

1. エディターで作業フォルダを開く（未作成なら先に §10 へ）
...
> [!TIP]
> **§4 の前に実施することを推奨** します。§4 で一度 `claude` を起動済みの場合は、以下で講義用フォルダに移動したあと、同じターミナルで `claude` を続行（または `/exit` 後に再実行）してください。

講義用フォルダを作り、その中で Claude Code を動かします。**エディターでも同じフォルダを開いておく** と、§11 以降のファイル操作の確認がしやすくなります。
...
すでにフォルダを作っている場合は、そのフォルダをエディターで開いたうえで、ターミナルでも同じ場所に移動（`cd`）してから `claude` を実行してください。
