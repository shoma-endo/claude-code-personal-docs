...
- `.claude/skills/meeting-summary/SKILL.md` — **§14**・**§21** の Skill 例
- `.claude/settings.json` — **§12** の設定ファイル例
- `meeting-memo.txt` — **§21** の議事メモ（ダミー）
- `examples/about-me.md` — **§18** の完成例（演習**後**に参照）
- `examples/meeting-summary.md` — **§21** の要約例
...
| **後半** | §16〜§22 | 同フォルダでファイル作成・要約・`CLAUDE.md` の追記 |
| **締め** | §23 | コマンドリファレンス、振り返り |
...
|:---:|---|---|
| 1 | オープニング、講義全体像 | 講義 |
| 2 | ターミナルとエディターの役割、講義の前提（CLI）、Claude Code とは | 講義・対話 |
| 3 | 個人利用の安全ルール | 講義 |
| 4 | **サンプルフォルダで**起動、初回対話、許可ダイアログ、停止方法 | 実習 |
| 5 | チェックポイント、エージェントループ | 講義 |
| 6 | **`CLAUDE.md`・`.claude/`・Skills** を実物で確認、Plan・コンテキスト | 講義・対話 |
| 7 | ファイル作成・読取・編集（`about-me.md`） | 実習 |
| 8 | 議事メモ要約、`CLAUDE.md` への追記 | 実習 |
| 9 | コマンドリファレンス、振り返り | 講義 |
...
- **後半（§17〜§22）**: 同じフォルダでファイル作成・要約・ルール追記の演習
...
> [!NOTE]
> **ルールの追記実習**は後半 **§22** で行います（ゼロから作るのではなく、既存の `CLAUDE.md` に 1 行足す）。
...
### 14. Skills / Commands / Agents / Hooks
...
- 長いファイルを一度に読みすぎない
- 調査や探索はサブエージェント（§14）に任せ、メインの文脈を節約する
...
| `meeting-memo.txt` | §21 の要約演習用（これから使う） |
...
続けて、要約を依頼します（前半 §14 の **meeting-summary** Skill の手順に沿っても構いません）。
...
> [!TIP]
> 2 行目は Skill を意識した依頼の例です。Skill 名の指定が難しい場合は、1 行目だけでも演習できます。
...
`/clear` のあと再度 `このプロジェクトのルールを教えてください` と送ると、追記が起動時に読み込まれることを確認できます。
